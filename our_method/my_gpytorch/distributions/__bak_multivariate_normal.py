...
class MultivariateNormal(TMultivariateNormal, Distribution):
    """
    Constructs a multivariate normal random variable, based on mean and cova riance.
    ...
    def log_prob(self, value: Tensor) -> Tensor:
        ...
        if covar.batch_shape:
            ...
            # print("before: ", covar.representation())
            covar = covar.evaluate_kernel()
            # print("diffshape", diff.shape)
            # print("diff unsqueeze shape", diff.unsqueeze(-1).shape)
            # print("after: ", covar.representation())
            # print("diff: ", diff)
            with settings.cholesky_jitter(1e-3):
                inv_quad, logdet = covar.inv_quad_logdet(inv_quad_rhs=diff, logdet=True)
