...
class DefaultPredictionStrategy(object):
    def __init__(self, train_inputs, train_prior_dist, train_labels, likelihood, root=None, inv_root=None):
        ...
        # print("exact strategy")
        # print("train labels", train_labels)
        # print("train labels shape", train_labels.shape)
        # print("len train shape", len(self.train_shape))
        # rint("train shape",self._train_shape)
        # print("train shape numel",self._train_shape.numel())
        try:
            #NP CHANGE
            train_labels = train_labels.reshape(
                *train_labels.shape[: -len(self.train_shape)-1], self._train_shape.numel()
            )
            # train_labels = train_labels.reshape(
    ...
    def _mean_cache(self, nan_policy: str) -> Tensor:
        ...
        if nan_policy == "ignore":
            print("train convar", train_train_covar)
            print("train_labels_offset", train_labels_offset)
            train_labels_offset = train_labels_offset.type('torch.FloatTensor')
    ...
    def exact_prediction(self, joint_mean, joint_covar):
        ...
        print("test mean", test_mean, test_mean.type())
        print("train covar", test_train_covar, test_train_covar.type())
        return (
